...
export default function EditProductPage() {
  ...
  function addRelated() {
    if (!data) return;
    setData({ ...data, relatedProducts: [...data.relatedProducts, { label: "", href: "" }] });
  }

  function removeRelated(index: number) {
    if (!data) return;
    setData({ ...data, relatedProducts: data.relatedProducts.filter((_, i) => i !== index) });
  }

  const pageHref = category === "loans" ? `/loans/${slug}` : `/insurance/${slug}`;
    ...
    <div className="min-h-screen" style={{ background: "var(--color-neutral-50)" }}>
      <div className="sticky top-0 z-40 border-b" style={{ background: "#fff", borderColor: "var(--color-neutral-200)", boxShadow: "0 1px 0 0 rgba(0,0,0,0.06)" }}>
        <div className="container-lh flex items-center justify-between py-3.5 gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/admin" className="flex items-center gap-1.5 text-sm font-semibold shrink-0 hover:opacity-70 transition-opacity" style={{ color: "var(--color-secondary)" }}>
              <ArrowLeft size={14} /> Admin
            </Link>
            <span style={{ color: "var(--color-neutral-300)" }}>/</span>
            <span className="text-sm font-semibold truncate" style={{ color: "var(--color-neutral-600)" }}>{data.title}</span>
          </div>

          <div className="flex items-center gap-2.5 shrink-0">
            <a href={pageHref} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm font-medium" style={{ color: "var(--color-primary)" }}>
        ...
        {saveStatus !== "idle" && (
          <div className="py-2 px-4 flex items-center gap-2 text-sm font-medium"
            style={{
              background: saveStatus === "success" ? "#ecfdf5" : "#fef2f2",
              color: saveStatus === "success" ? "#065f46" : "#991b1b",
              borderTop: saveStatus === "success" ? "1px solid #a7f3d0" : "1px solid #fecaca",
            }}>
            {saveStatus === "success" ? (
              <><CheckCircle size={14} /> Changes saved successfully! The page is updated.</>
            ) : (
              <><AlertCircle size={14} /> {errorMsg || "Failed to save. Check your session or try again."}</>
            )}
          </div>
        )}
      </div>
      ...
      <div className="container-lh max-w-3xl py-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--color-primary)" }}>{category}</p>
            <h1 className="text-xl font-bold" style={{ fontFamily: "var(--font-display)", color: "var(--color-secondary)" }}>{data.title}</h1>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="card p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input id="f-badge" type="text" className="input-lh" value={data.badge}
                  onChange={(e) => setData({ ...data, badge: e.target.value })} placeholder="e.g. Fast Approval" />
              </div>
              <div>
                <label className="label-lh" htmlFor="f-title">Page Title</label>
                <input id="f-title" type="text" className="input-lh" value={data.title}
                  onChange={(e) => setData({ ...data, title: e.target.value })} placeholder="e.g. Personal Loan" />
              </div>
              <div className="sm:col-span-2">
                <label className="label-lh" htmlFor="f-tagline">Tagline</label>
                <input id="f-tagline" type="text" className="input-lh" value={data.tagline}
                  onChange={(e) => setData({ ...data, tagline: e.target.value })} placeholder="Short catchy line under the title" />
              </div>
              <div className="sm:col-span-2">
                <label className="label-lh" htmlFor="f-description">Description</label>
                <textarea id="f-description" className="input-lh" style={{ minHeight: "100px", resize: "vertical" }}
              ...
              <div>
                ...
                <input id="f-cta-label" type="text" className="input-lh" value={data.ctaLabel}
                  onChange={(e) => setData({ ...data, ctaLabel: e.target.value })} placeholder="e.g. Apply Now" />
              </div>
              <div>
                <label className="label-lh" htmlFor="f-cta-href">CTA Button Link</label>
                <input id="f-cta-href" type="text" className="input-lh" value={data.ctaHref}
                  onChange={(e) => setData({ ...data, ctaHref: e.target.value })} placeholder="/apply" />
              </div>
            </div>
          </div>

          <ArrayEditor title="Key Features" items={data.features} placeholder="Enter a feature"
            onUpdate={(i, v) => updateArrayItem("features", i, v)}
            onAdd={() => addArrayItem("features")}
            onRemove={(i) => removeArrayItem("features", i)} />

          <ArrayEditor title="Eligibility Criteria" items={data.eligibility} placeholder="Enter an eligibility criterion"
            onUpdate={(i, v) => updateArrayItem("eligibility", i, v)}
            onAdd={() => addArrayItem("eligibility")}
            onRemove={(i) => removeArrayItem("eligibility", i)} />

          <ArrayEditor title="Documents Required" items={data.documents} placeholder="Enter a document"
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={addRelated} className="flex items-center gap-1.5 text-sm font-semibold btn btn-secondary" style={{ padding: "0.4rem 0.875rem" }}>
                <Plus size={14} /> Add
              </button>
            </div>
            <div className="flex flex-col gap-3">
              {data.relatedProducts.map((rp, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <input type="text" className="input-lh flex-1" placeholder="Label (e.g. Business Loan)" value={rp.label}
                  ...
                  <button onClick={() => removeRelated(i)} className="shrink-0 p-2 rounded-lg transition-colors hover:bg-red-50"
                    style={{ color: "#dc2626" }} aria-label="Remove related product">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
          ...
          <button onClick={handleSave} disabled={saving} className="btn btn-primary w-full flex items-center justify-center gap-2 mt-2">
...
function ArrayEditor({ title, items, placeholder, onUpdate, onAdd, onRemove }: {
      ...
      <div className="flex flex-col gap-2.5">
        {items.map((item, i) => (
          <div key={i} className="flex gap-2 items-center">
            ...
            <button onClick={() => onRemove(i)} className="shrink-0 p-2 rounded-lg transition-colors hover:bg-red-50"
              style={{ color: "#dc2626" }} aria-label={`Remove item ${i + 1}`}>
              <Trash2 size={15} />
            </button>
          </div>
        ))}
        {items.length === 0 && (
          <p className="text-sm" style={{ color: "var(--color-neutral-400)" }}>No items yet. Click Add to add one.</p>
        )}
      </div>
    </div>
  );
